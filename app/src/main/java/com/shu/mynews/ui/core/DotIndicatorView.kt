...
@JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private var propertyRotate =
        PropertyValuesHolder.ofInt(RADIUS_ANIMATION_PROPERTY, 0, DEFAULT_RADIUS_DP)
    private var propertyUncheck =
        PropertyValuesHolder.ofInt(RADIUS_ANIMATION_PROPERTY, DEFAULT_RADIUS_DP, 0)
            ...
            when {
                ...
                inputtedCounter < i -> {
                    ...
                    if (afterUncheck && i - inputtedCounter == 1) {
                        canvas.drawCircle(
                            firstDotX + i * diameterWithMargin,
                            firstDotY,
                            dip2px(radius),
                            selectedPaint
                        )
    ...
    private fun measureWidth(measureSpec: Int): Int {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result =
                (paddingLeft + paddingRight + numberOfDots * 2 * dip2px(DEFAULT_RADIUS_DP) + (numberOfDots - 1) * dip2px(
                    DOT_MARGIN
                )).toInt()
